...
void init_patches(hvr_vertex_t*& patches, hvr_ctx_t ctx) {
    ...
    unsigned const_features[8] = {7, 8, 9, 10, 11, 12, 13, 14};
    double const_vals[8] = {2, 4, 6, 8, 1.0, 0.9, 0.8, 0.7};
    ...
    for (uint64_t i = 0; i < patches_per_pe; i++) {
        ...
        hvr_vertex_set(0, x, patches + i, ctx);
        hvr_vertex_set(1, y, patches + i, ctx);

        // Dynamic attributes (5 of them):
        // Store Ihh & Nhh for the record?
        hvr_vertex_set(2, 0.0, patches + i, ctx);       // lambda_h
        hvr_vertex_set(3, Kv / 2.0, patches + i, ctx);  // Sv
        hvr_vertex_set(4, 0.0, patches + i, ctx);       // Ev
        hvr_vertex_set(5, 0.0, patches + i, ctx);       // Iv
        hvr_vertex_set(6, Kv / 2.0, patches + i, ctx);  // Nv = Sv + Ev + Iv

        for (unsigned j = 0; j < 8; j++) {
            hvr_vertex_set(const_features[j], const_vals[j], patches + i, ctx);
        }
...
void init_agents(hvr_vertex_t*& agents, hvr_ctx_t ctx) {
    ...
    for (uint64_t i = 0; i < agents_per_pe; i++) {
        ...
        hvr_vertex_set(2, x, agents + i, ctx);
        hvr_vertex_set(3, y, agents + i, ctx);
        ...
        // Dynamic attributes (5 of them):
        hvr_vertex_set(0, x, agents + i, ctx);    // Current x
        hvr_vertex_set(1, y, agents + i, ctx);    // Current y
        hvr_vertex_set(4, h, agents + i, ctx);    // Health condition
        hvr_vertex_set(5, double(act_id), agents + i, ctx);  // ID of current activity
        hvr_vertex_set(6, 0.0, agents + i, ctx);  // Time steps stayed at current activity
...
void update_metadata(hvr_vertex_t *vec,
    ...
    if ((int)hvr_vertex_get(GRAPH_FEAT, vec, ctx) == PATCH_GRAPH) { // This is a patch
        ...
        for (size_t i = 0; i < n_neighbors; i++) {
            ...
            const double h = hvr_vertex_get(4, neighbor, ctx);
            const uint64_t act = hvr_vertex_get(5, neighbor, ctx);
            Nh[act] += 1;
        ...
        double Sv = hvr_vertex_get(3, vec, ctx);
        double Ev = hvr_vertex_get(4, vec, ctx);
        double Iv = hvr_vertex_get(5, vec, ctx);
        double Nv = hvr_vertex_get(6, vec, ctx);

        // Get relative risks of being bitten
        const double alpha_0 = hvr_vertex_get(11, vec, ctx);
        const double alpha_1 = hvr_vertex_get(12, vec, ctx);
        const double alpha_2 = hvr_vertex_get(13, vec, ctx);
        const double alpha_3 = hvr_vertex_get(14, vec, ctx);
        ...
        hvr_vertex_set(2, lh, vec, ctx);
        hvr_vertex_set(3, Sv, vec, ctx);
        hvr_vertex_set(4, Ev, vec, ctx);
        hvr_vertex_set(5, Iv, vec, ctx);
        hvr_vertex_set(6, Nv, vec, ctx);
    ...
    if ((int)hvr_vertex_get(GRAPH_FEAT, vec, ctx) == AGENT_GRAPH) { // This is an agent
        ...
        const double health = hvr_vertex_get(4, vec, ctx);
        const int act_id = hvr_vertex_get(5, vec, ctx);
        const double alpha = hvr_vertex_get(act_id + 11, patch, ctx);
...
void update_coupled_val(hvr_vertex_iter_t *iter,
    ...
    hvr_vertex_t* v = hvr_vertex_iter_next(iter);
    while (v) {
